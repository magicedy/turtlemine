...
namespace TurtleMine
{
    ...
    internal class FormSettings
    {
        ...
        /// <summary>
        /// Renames the old settings from versions prior to 1.1.0.5 and versions prior to 1.2.0.1.
        /// </summary>
        private static void renameOldSettings()
        {
            renameRegistrySection("Software\\TortoiseSVNRedminePlugin", RegAdressBase);
            renameRegistrySection("Software\\TortoiseRedminePlugin", RegAdressBase);
        }
        ...
        //Registry settings
        private const string RegAdressBase = "Software\\TurtleMine";
        private const string RegAdressRecent = "Software\\TurtleMine\\RecentID";
        private const string RegAdressColumnsDisplay = "Software\\TurtleMine\\ColumnsDisplay";
        ...
        public static string[] GetRecentItems()
        {
            ...
            if (myReg != null)
            {
                ...
                recentItems.AddRange(myReg.GetValueNames().Select(item => (string) myReg.GetValue(item)));
            }
        ...
        /// <returns></returns>
        private static RegistryKey openRegistryKey(string key, bool createIfNotExists = false, bool deleteExistingKeyFirst = false)
        {
        ...
        /// <summary>
        /// Renames the registry section.
        /// </summary>
        /// <param name="oldAddress">The old address.</param>
        /// <param name="newAddress">The new address.</param>
        private static void renameRegistrySection(string oldAddress, string newAddress)
        {
            //Try and open the old reg settings location
            var myReg = openRegistryKey(oldAddress);
            ...
            using (var destinationKey = openRegistryKey(newAddress, true))
            {
                //Open the sourceKey we are copying from
                using (var sourceKey = openRegistryKey(oldAddress))
                {
            ...
            //Remove old key
            Registry.CurrentUser.DeleteSubKeyTree(oldAddress);
        }
